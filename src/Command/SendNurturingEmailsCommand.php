 ...
 */
#[AsCommand(
    name: 'app:send-nurturing-emails',
    description: 'Envoie les emails de nurturing aux leads (J+1, J+3, J+7)',
)]
class SendNurturingEmailsCommand extends Command
{
    ...
    {
        $this
            ->addOption('dry-run', null, InputOption::VALUE_NONE, 'Affiche les emails qui seraient envoyés sans les envoyer réellement')
            ->addOption('day', 'd', InputOption::VALUE_REQUIRED, 'Ne traiter que les emails d\'un jour spécifique (1, 3 ou 7)')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($leads as $lead) {
            ...
            if (($specificDay === null || $specificDay === 1) && $lead->shouldReceiveNurturingDay1()) {
                if ($dryRun) {
                    $io->text(sprintf('[DRY RUN] J+1 : %s (%s) - %d jours depuis création', $lead->getEmail(), $lead->getFullName(), $lead->getDaysSinceCreation()));
                } else {
            ...
            if (($specificDay === null || $specificDay === 3) && $lead->shouldReceiveNurturingDay3()) {
                if ($dryRun) {
                    $io->text(sprintf('[DRY RUN] J+3 : %s (%s) - %d jours depuis création', $lead->getEmail(), $lead->getFullName(), $lead->getDaysSinceCreation()));
                } else {
            ...
            if (($specificDay === null || $specificDay === 7) && $lead->shouldReceiveNurturingDay7()) {
                if ($dryRun) {
                    $io->text(sprintf('[DRY RUN] J+7 : %s (%s) - %d jours depuis création', $lead->getEmail(), $lead->getFullName(), $lead->getDaysSinceCreation()));
                } else {
        ...
        if ($dryRun) {
            ...
            $io->success(sprintf('Commande terminée. %d emails de nurturing dispatchés.', $stats['day1'] + $stats['day3'] + $stats['day7']));
        }
    ...
    {
        $message = new SendLeadNurturingEmailMessage(
            leadId: $lead->getId(),
            dayNumber: $dayNumber,
        );
