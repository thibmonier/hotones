...
class CompanyCrudController extends AbstractCrudController
{
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        yield TextField::new('name', 'Nom')->setRequired(true)->setHelp('Nom de la société');

        yield TextField::new('slug', 'Slug')->setRequired(true)->setHelp(
            'Identifiant unique pour les URLs (lettres minuscules, chiffres et tirets uniquement)',
        );
        ...
        // Limits
        yield IntegerField::new('maxUsers', 'Max utilisateurs')->setHelp(
            'Nombre maximum d\'utilisateurs (null = illimité)',
        )->hideOnIndex();

        yield IntegerField::new('maxProjects', 'Max projets')->setHelp(
            'Nombre maximum de projets (null = illimité)',
        )->hideOnIndex();

        yield IntegerField::new('maxStorageMb', 'Max stockage (Mo)')->setHelp(
            'Stockage maximum en Mo (null = illimité)',
        )->hideOnIndex();

        // Billing
        yield DateField::new('billingStartDate', 'Date début facturation')->hideOnIndex();

        yield IntegerField::new('billingDayOfMonth', 'Jour facturation')->setHelp(
            'Jour du mois pour la facturation (1-28)',
        )->hideOnIndex();

        yield TextField::new('currency', 'Devise')->setHelp('Code devise ISO (ex: EUR, USD)')->hideOnIndex();

        // Features
        yield ArrayField::new('enabledFeatures', 'Fonctionnalités activées')->setHelp(
            'Features: '
                .implode(', ', [
        ...
        yield IntegerField::new('annualPaidLeaveDays', 'Jours de congés payés')->setHelp(
            'Nombre de jours de congés payés par an (défaut: 25)',
        )->hideOnIndex();

        yield IntegerField::new('annualRttDays', 'Jours RTT')->setHelp(
            'Nombre de jours RTT par an (défaut: 10)',
        )->hideOnIndex();
        ...
        yield DateTimeField::new('trialEndsAt', 'Fin d\'essai')->hideOnForm()->hideOnIndex();

        yield DateTimeField::new('suspendedAt', 'Suspendu le')->hideOnForm()->hideOnIndex();
    }
