...
class SaasProviderCrudController extends AbstractCrudController
{
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        yield TextField::new('name', 'Nom')
            ->setRequired(true)
            ->setHelp('Nom du fournisseur (ex: Google, Microsoft, Adobe)');

        yield UrlField::new('website', 'Site web')
            ->setHelp('URL du site web du fournisseur');
        ...
        yield UrlField::new('logoUrl', 'URL du logo')
            ->setHelp('URL ou chemin vers le logo du fournisseur')
            ->hideOnIndex();

        yield TextareaField::new('notes', 'Notes')
            ->hideOnIndex()
            ->setHelp('Notes internes sur le fournisseur');
